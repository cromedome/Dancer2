...
F<development.ext>, for example).  Dancer2 will next look for any developer- 
specific environment configuration files (files prefixed with F<my->, such as
F<my-production.ext>).  Any settings in these files that are
named the same as settings in your global configuration file will take
precedence over those settings in the global file, and the rest of the 
settings are merged.

If there's any additional configuration needed (besides what's in your
development or environment configuration files), you can pass the complete
path and filename through the C<DANCER_ALT_CONFIG> environment variable. This
file may contain database credentials or other information you don't want 
living in your application's version control repository. Any settings in this
file that are named the same as a setting in your development or environment
configuration files take precedence over earlier settings. The rest of the 
settings are merged, and Dancer2 uses the combination of settings from these
files as its operating configuration.
...
If you want to force your Dancer application to use a particular type of 
configuration file, you can set the environment variable C<DANCER_CONFIGFILE_EXT> 
to the extension you wish to use (C<json>, C<yaml>, etc.). The extension must
be a filetype recognized by L<Config::Any>.
